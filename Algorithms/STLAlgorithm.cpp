...
int main()
{
     ...
     * 가능한 모든 순열 생성하기
     */
    sort(v1.begin(), v1.end()); // 순열을 생성하기 전에 대이터가 사전 순으로 정렬된 상태야 함.
    ...
    */
     ...
     */
     ...
     // 중복되는 요소를 뒤로 밀어낸 뒤 중복되지 않는 새로운 범위의 끝 반복자를 반환(O(n))
    auto new_end = unique(v1.begin(), v1.end()); 

    // 중복되지 않은 요소들만 출력 (1, 2, 3)
    for(auto it = v1.begin(); it != new_end; it++)
    {
        cout << *it << " ";
    }

    sort(v1.begin(), v1.end()); // 이미 정렬되어 있어야 정상 작동
